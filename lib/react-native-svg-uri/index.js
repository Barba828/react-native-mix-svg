...
class SvgUri extends Component {
  constructor(props) {
    super(props);
    const { fill, svgXmlData, source = null } = props;
    this.state = { fill, svgXmlData, source };
    if (source) {
      const source = resolveAssetSource(source) || {};
   ...
   */
  inspectNode = (node) => {
    // Only process accepted elements 非有效 svg 标签
